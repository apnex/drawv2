/*
GR10 — the interface record never asserts the opposite of the running wire.

This is the check that did not exist. CS1 and CS3 each reversed a locked decision without
amending the interface record, and nothing failed: the file went unchanged from genesis through four
...
`server/protocol.js`'s own dispatch and compared against what the interface record documents. Adding a `case`
...
Concatenated rather than re-pointed one assertion at a time, deliberately. Splitting a proven
contract is the moment a test is most likely to be quietly narrowed to keep a gate green, and a
single source keeps every assertion reading exactly what it read before.
*/
const scope = [
...
function section(heading) {
	...
	assert.notEqual(start, -1, `no "## ${heading}" section in API.md or DECISIONS.md`);
...
test('GR10: every command the server dispatches is documented in the interface record', () => {
...
test('GR10: the interface record documents no command the server has stopped answering', () => {
	...
	assert.deepEqual(dead, [], `the interface record still documents commands the server refuses: ${dead.join(', ')}`);
});
...
test('GR10: the retired wire tokens are gone from the reference, not merely annotated', () => {
	...
	for (const dead of ['cmd:"push"', 'cmd:"apply"', 'body:{action:"put"']) {
		assert.equal(scope.includes(dead), false, `the interface record still prints the dead wire form ${dead}`);
	}
});
...
test('GR10: every locked decision this arc reversed carries a dated amendment', () => {
	...
	assert.match(section('Capability rulings, and what reversed them'), /undo\/redo are a SERVER capability/i);

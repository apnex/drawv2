...
| H14.10 | A beat committed after the queue drained inherits an expired schedule and never unfurls -- and the caption still shows, so it reads as working. The origin must reset when the schedule has finished: a beat arriving into a drained queue is starting one, not joining it | **B193** | S2 - S | `TODO` |

...
| H11.18 | Authoring the reference topology needed eleven departures from the tool -- loops, `python3` over `--json`, a heredoc per panel, a headless browser to see the result. Every verb answers with a table, so an agent cannot LOOK at the canvas it is editing | **B139** | S2 · L | `TODO` |

...
| H14.4 | A beat: a set plus pacing and a caption, revealed by derivation from one stored origin. The caption plays in the menu bar as the second channel, and cancel drops the reveal while leaving the ops | **B188** | S3 - L | `WIP` |
| H14.7 | Author a beat: `--pace` and `--caption` on `commit`, the reveal record built SERVER-side because only the planner knows which ids an intent op produced, and a ruling on whether undoing a beat takes its reveal with it | **B188** | S2 - M | `TODO` |

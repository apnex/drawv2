...
| H12.10 | The pilot's AGENT surface: a verb that mints a waypoint-terminated link, a verb that arms an endpoint, and `add` refusing a type no glyph exists for. Found by trying to build this milestone's own fixture through `draw` and failing at every step | **B167**, **B168** | S3 - M | `TODO` |

...
| H9.36 | An agent's writes did not reach a watching browser though every one was durable and broadcast — and GR6 asserts they do, passing against a harness rather than a real client | **B106** | S3 · M | `TODO` |
| H9.36 | A watching browser silently discards every remote change — the staleness guard reads a version advanced two lines earlier, so live collaboration has never worked | **B106** | S3 · M | `TODO` |
| H9.37 | The convergence harness reimplements the client's change rule rather than calling it, and its copy is the correct one — GR6 passes over code that does not run | **B107** | S3 · M | `TODO` |

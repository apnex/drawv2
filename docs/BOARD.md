...
| H14.6 | `--draft` on every write verb, not just `place`. Twelve of thirteen silently ignore it and write immediately, so a staged set half-applies with nothing saying so -- measured on the estate after the deploy. Includes the wider question the defect exposes: an unrecognised flag is dropped rather than refused | **B190** | S2 - M | `TODO` |

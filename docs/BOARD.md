...
| H14.9 | A one-entity beat has zero duration, so its caption is never read -- an agent writes one, the commit takes it, the document keeps it, and nothing shows it. A beat's DURATION and its entity spacing are different questions and `durationOf` answers both | **B192** | S2 - S | `TODO` |

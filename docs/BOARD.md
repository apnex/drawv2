...
| H14.5 | One sovereign resolver over `(model, layout, relationship)` returning an anchor, so a transaction can choose a position. The server answers occupancy but never SELECTS -- the rules live in `cli/verbs.mjs` alone, which is why `place` costs two round trips and why `plan()` cannot resolve an intent op today | **B189** | S2 - S | `TODO` |

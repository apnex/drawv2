...
| H14.5 | One sovereign nearest-anchor resolver, imported by the REST anchor route and by `planOne`, so a transaction and a query answer the same question with the same code. `cli/verbs.mjs:888` and `server/rest.js:642` are already twins and `scan-twins` cannot see them -- both loops sit inside larger functions, so the guard for exactly this gives no signal | **B189** | S2 - S | `TODO` |

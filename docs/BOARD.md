...
| H14.2 | A set: ops drafted locally with the SAME verbs an agent already uses, then committed as one call. The atomic transport exists -- `commit --ops` applies a batch as one transaction and `CLI.md` rules it stays -- so the gap is the draft. BLOCKED on W7: a write verb RESOLVES against live state (44 call sites), so two drafted `place near lb-1` ops pick the same anchor and the set is refused whole. Measured on the estate 2026-09-04 | **B188** | S3 - M | `BLOCKED` |

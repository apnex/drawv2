...
This generation restores what prism removed (user interactivity) and removes what prism proved but this tool does not need (the 4-layer compiler, distributed convergence, auto-layout).\
Coordinates are user-owned.\
References are mined for narrow mechanisms only.

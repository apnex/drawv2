...
`dev/history/COMMIT-AUDIT.md` preserves the axiom-alignment audit run against the design before any of it was implemented.
...
Recorded as deviation **X2**; `dev/BACKLOG.md` **B6** carries the revival trigger (any multi-instance or GCS-backed deployment - confirm the gcsfuse assumptions against a real bucket before relying on them).

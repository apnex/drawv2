...
Framing: draw is an ENGINEERING tool, not a drawing tool - strict geometric precision, interaction language inspired by Factorio/Shapez.\
The shared bar for every proposal: **zero ambiguity between intent and result - the machine states what will happen, in numbers, before commit.**

Method: four independent design lenses (Factorio/Shapez conventions, CAD/EDA precision tools, ergonomics audit of the current UI, keyboard-first operation) each grounded in the actual codebase -> 23 proposals -> deduplicated to 18 -> scope-checked against SCOPE.md.\
...
2. **Ortho lock** *(SHIPPED 2026-06-12)* (2 lenses) - Shift mid-move/clone constrains to the dominant axis
   (AutoCAD ORTHO); readout gains delta-in-cells for every move. One-line delta change. Cost S.
...
15. **Pair-distance readout** *(always-on half SHIPPED 2026-06-13)* - ship the always-on half (2 selected -> delta + length in the
...
digits 1-6 + Q = stamp hand - Space = datum - Z = wrap-zone - Ctrl+D = duplicate - Shift mid-drag = ortho - Shift at release (link mode) = chain - Shift+arrows = zone resize - Tab = contested (rename-run inside editor is free; canvas Tab held for data view) - Enter = contested (coordinate entry vs stamp-at-ghost) - L/Shift+L = link selected - Alt+left-drag = delete marquee - M = unbound (measure mode cut)

...
Running record of the GRC walk (style × rung). Failures are kept (history) and each
generates a "what would make it pass" hypothesis → new candidate styles/rungs.
Criteria: grid-snap · clearance · attachment · reserve · count-honored (`design/walk/grc.mjs` + run.mjs).

## Walk 1 — rungs 1–5 × {ports, hybrid, nodeAligned, edgeNode}
...
- **Refuted:** `outer-flank gutters` — for a tight zone (= cols + ext) the flank positions fall
  *outside* the zone edge, so node-agnostic genuinely caps at cols−1. Higher count ⇒ more columns
  (wider topology) or a column-based style.
- **Remediation applied (gap-row snap):** `gatewayCell`/`bus` now place the bridge at the nearest
  on-grid node row, not `(from+to)/2`. Result: **`gatewayCell` = clean sweep on all 8 rungs** (joins
  `nodeAligned`); `bus` grid-snap cleared (only the count cap remains). The failure→remediation loop
  produced a winning survivor (`gatewayCell`, born from edge-node's failure).

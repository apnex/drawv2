...
| H13.15 | Every entity carries a name, mandatory on all five kinds, so anything addressable can be referred to in words. Waypoints and links were the gap | **B187** | S2 - M | `DONE` |
| H13.18 | The server remembers what each client DID, and keeps the OPENING of a session rather than only its tail -- so an incident can be read after the fact instead of watched live. A night of investigation produced no cause because every capture began mid-flood | **B182** | S2 - M | `DONE` |
| H13.16 | A session may not write faster than a gesture can produce, bounded server-side where a client fix cannot bypass it -- and a refusal must DAMP the flood rather than demand a snapshot per rejection. The P0: 78 transactions per second, version 4862 to 144961 in an hour, chased for three deploys as four different defects it was not | **B181** | S1 - M | `DONE` |
| H13.17 | `draw use` selects the diagram every other verb acts on, keyed by host because an id is only meaningful on the server holding it -- and a write with no selection REFUSES rather than guessing. Fourteen nodes went into the wrong document during a rehearsal before anything said so | **B186** | S2 - S | `DONE` |
...
| H14.2 | A set: ops drafted locally with the SAME verbs an agent already uses, then committed as one call. The atomic transport exists -- `commit --ops` applies a batch as one transaction and `CLI.md` rules it stays -- so the gap is the draft. A write verb RESOLVES against live state (44 call sites), so two drafted `place near lb-1` ops pick the same anchor and the set is refused whole -- measured on the estate. W7 ruled INTENT-DRIVEN: the op stores what was expressed and the server resolves at commit, so `cli/` stays standalone. `--draft` stages, `draw commit` sends the set as one transaction, and a drafted `place` emits a relationship rather than a position | **B188** | S3 - L | `DONE` |
| H14.5 | One sovereign resolver over `(model, relationship)` returning an anchor, so a transaction can choose a position. The server answered occupancy but never SELECTED -- the rules lived in `cli/verbs.mjs` alone, which is why `place` costs two round trips and why `plan()` could not resolve an intent op. `planOne` now resolves a `place` op through it, and two intents in one transaction land on different anchors | **B189** | S2 - S | `DONE` |
| H14.6 | `--draft` on every write verb, not just `place`. Twelve of thirteen silently ignored it and wrote immediately, so a staged set half-applied with nothing saying so. The dispatcher now refuses any flag a verb does not declare, which is the B161 rule applied to flags rather than positionals -- it caught a real misuse on its first live invocation | **B190** | S2 - M | `DONE` |
...
| H15.13 | The CLI shows what the document holds, so no field is invisible to the tool | **B231** | S3 | `DONE` |
| H15.14 | A pass-through is a BEND -- the matrix case the code never decided | **B232** | **S2** | `DONE` |
...
Two opposing declarations on one run do not fight.\
The waypoint between them holds two links that oppose, which the matrix already calls a junction, so the run breaks there and the conflict is visible exactly where it was authored.

This is the general rule arriving where it applies rather than a new one, and it keeps the alternative's failure out: latest-wins would silently rewrite an authored decision whose only evidence is a change somewhere off-screen, which is the shape B220 had.

The consequence for H15.3 is that a declaration stores only ITSELF.\
No ordering, no timestamps, no precedence -- the conflict is resolved by geometry rather than by history.

**H15.3 and H15.4 landed together**, because the rung boundary was wrong.\
`facing` is a primitive with no consumer until something asks what the directions MAKE of a waypoint, and the dead-code scanner said so: the field and its reading were test-only until the matrix arrived to use them.\
The gesture and the arrowhead split out as H15.6 instead -- they are the surface, and the rule underneath them is now provable without one.
...
Nor the DECOMPOSITION of the glyphs into dynamic visual mechanisms, which the framework enables rather than requires -- once the pipeline exists that becomes pack design, not a change to the mechanism.\
...
CORRECTED 2026-09-25: first recorded as met on CI run 36089220400; the next push failed the same teardown (run 36109819709), so B238 and this milestone are open again until several consecutive green gate runs. The collapse and sweep fixes stand, held by the tests and mutants recorded in B239-B241. The review's one held finding is B242.
---
